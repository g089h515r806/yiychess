...
Page({
  ...
  onReady: function (e) {
	...
	var uuid = '1f816196-906d-46b6-9457-f1ae41a44657';
	*/
	var that = this; 
    wx.request({
          url: 'https://chess.yaiyuan.com/jsonqipu/' + that.data.uuid,
          method: 'GET',
	...
	var fen_list = util.getFenList(qipu.field_fen, qipu.field_moves_iccs)
	//this.setData({
    //    fen_list:fen_list
    //})
	
	var total_steps = Math.floor(qipu.field_moves_iccs.length / 4);
	that.setData({
		qipu:qipu,
		fen_list:fen_list,
	...
	util.qipanDrawing(context,offsetX, offsetY, cellSize)
	util.fenDrawing(context, fen_list[that.data.current_step], offsetX, offsetY, cellSize)
